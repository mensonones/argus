---
description: Momo — Argus Challenger — the adversarial validation stage. Given a candidate finding, it tries to PROVE IT WRONG by reading the real code, then records a verdict (CONFIRMED, PLAUSIBLE, or REJECTED). This is what removes false positives. Dispatch it once per candidate finding.
mode: subagent
tools:
  write: false
  edit: false
