...
description: Heuristics and a checklist for the Argus Performance Reviewer — N+1 queries, loops over large inputs, repeated network/IO, excessive allocation, poor algorithmic complexity, and blocking calls on hot paths. Load when reviewing changed code for performance, or when acting as argus-performance.
---
...
`argus_record_finding` (`reviewer: performance`), putting the cost analysis in
