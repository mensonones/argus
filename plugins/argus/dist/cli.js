...
import { initReview, recordFinding, recordChallenge, recordReviewerRun, listFindings, memorySearch, importBaseline, suppressFinding, listSuppressions, updateGlobalMemory, report, } from "./service.js";
    ...
    try {
        switch (command) {
            ...
            case "reviewer-run": {
                ...
                recordReviewerRun(cwd, reviewer, status, values.detail);
            ...
            case "challenge": {
                ...
                const ok = recordChallenge(cwd, id, verdict, values.reason ?? "");
            ...
            case "memory-status": {
                ...
                const updated = updateGlobalMemory(fingerprint, status, values.note);
            ...
            case "report": {
                ...
                if (out.exportPath)
                    console.error(color.dim(`written to ${out.exportPath}`));
                return out.result.findings.some((f) => f.severity === "high" || f.severity === "critical")
                    ? 2
                    : 0;
            }
            default:
