...
export async function detectStack(repoRoot, changedPaths) {
    ...
    for (const file of [...changedPaths].sort()) {
        ...
        if (path.isAbsolute(normalized) || parts.some(p => p === ".." || BLOCKED.has(p)))
            continue;
        let dir = path.posix.dirname(normalized);
        while (dir !== ".") {
            if (directories.size >= MAX_DIRECTORIES && !directories.has(dir)) {
                truncated = true;
                break;
            }
            directories.add(dir);
    ...
    const result = { signals: [], manifests: [], warnings: [], truncated };
    for (const dir of [...directories].sort()) {
        ...
        try {
            ...
            if (relative.startsWith(`..${path.sep}`) || relative === ".." || path.isAbsolute(relative)) {
                result.warnings.push(`${manifest}: target outside repository skipped`);
                continue;
            }
            handle = await fs.open(real, "r");
            const stat = await handle.stat();
            if (!stat.isFile() || stat.size > 1024 * 1024) {
                result.warnings.push(`${manifest}: not a regular manifest or exceeds 1 MiB`);
                continue;
            }
            const pkg = JSON.parse(await handle.readFile("utf8"));
            ...
            for (const section of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"]) {
                ...
                for (const [dep, value] of Object.entries(deps)) {
                    ...
                    if (kind)
                        result.signals.push({ kind, name: FRAMEWORKS[dep] ?? RUNNERS[dep], manifest, evidence: `${section}.${dep}` });
                }
            }
            if (typeof data.packageManager === "string") {
                const manager = /^(npm|pnpm|yarn|bun)@[^\s]+$/.exec(data.packageManager)?.[1];
                if (manager)
                    result.signals.push({ kind: "package-manager", name: manager, manifest, evidence: "packageManager" });
            }
            ...
            if (err.code !== "ENOENT")
                result.warnings.push(`${manifest}: unreadable or invalid JSON`);
        }
        finally {
            await handle?.close();
        }
