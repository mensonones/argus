...
export function consolidateReconciled(findings, groups, requireCoverage = false) {
    ...
    const result = groups.map(group => {
        ...
        if (members.length > 1 && requireCoverage) {
            if (!group.causal_analysis || !group.claim_coverage)
                throw new Error("Merged groups require causal_analysis and claim_coverage for canonical consequences.");
