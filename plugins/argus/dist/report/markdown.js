...
function renderFinding(f, index) {
    const lines = [];
    lines.push(`### ${index}. ${SEVERITY_LABEL[f.severity]} · ${f.category} — ${f.title}`);
    lines.push("");
    lines.push(`**Location:** \`${location(f)}\``);
    if (f.sourceCommits?.length)
        lines.push(`**Source commits:** ${f.sourceCommits.map(sha => `\`${sha}\``).join(", ")}`);
    lines.push("");
    if (f.description) {
    ...
    if (f.rootCause)
        lines.push(`**Root cause:** ${f.rootCause.symbol} · ${f.rootCause.mechanism} · ${f.rootCause.invariant}`);
    if (f.corrections?.length) {
        lines.push("", "**Challenger corrections**", "");
        for (const correction of f.corrections)
    ...
    if (packet) {
        ...
        for (const item of packet.preconditions)
            lines.push(`- Precondition: ${item}`);
        lines.push(`- Expected: ${packet.expected}`, `- Observed: ${packet.observed}`);
        if (packet.command)
            lines.push(`- Command (not executed by Argus): ${packet.command}`);
        if (packet.artifact)
            lines.push(`- Recorded result/artifact: ${packet.artifact}`);
        if (packet.negativeControl)
            lines.push(`- Negative control: ${packet.negativeControl.scenario} — ${packet.negativeControl.observed}`);
        for (const item of packet.limitations)
            lines.push(`- Limitation: ${item}`);
    }
    if (f.baselineStatus)
...
export function renderMarkdown(result) {
    ...
    if (result.scope?.selectedCommits)
        out.push(`- Selected non-merge commits (${result.scope.selectedCommits.length}): ${result.scope.selectedCommits.map(sha => `\`${sha}\``).join(", ") || "none"}`);
    if (result.scope?.selectionReason)
        out.push(`- Scope selection: ${result.scope.selectionReason}`);
