 ...
 */
export function abandonRound(cwd, roundId, reason) {
    if (!path.isAbsolute(cwd))
        throw new Error("repo_path must be absolute.");
    const repoRoot = repoRootSync(cwd);
    const mem = Memory.open(repoRoot);
...
export function recordChallenge(cwd, findingId, result, reasoning, evidencePackage, correction, rootCause, execution) {
    ...
    return withCurrentRound(cwd, (mem, roundId) => mem.updateChallenge(roundId, findingId, result, reasoning, packet, corrected, cause, run), true);
}
...
export function report(opts) {
    ...
    try {
        ...
        const saved = savedCheck ? JSON.parse(savedCheck) : undefined;
        const check = opts.provenance ?? (saved?.signature === findingsSignature(all) ? saved.provenance : undefined);
        if (delegated.length) {
            if (!check)
                throw new Error("Before report, supply provenance with coordinator_id and dispatched_challenger_ids from actual host dispatch outputs.");
        ...
        if (check)
            mem.setMeta(`provenance-check:${round.id}`, JSON.stringify({ signature: findingsSignature(all), provenance: check }));
