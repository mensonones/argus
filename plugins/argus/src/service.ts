...
export function reviewContext(cwd: string, roundId: string) {
  if (!path.isAbsolute(cwd)) throw new Error("repo_path must be absolute.");
  const repoRoot = repoRootSync(cwd);
  if (!fs.existsSync(targetDbPath(repoRoot))) throw new Error("No Argus database in this repository; attach cannot initialize one.");
  const mem = Memory.open(repoRoot);
  try {
    const round = mem.getRound(roundId);
    if (!round) throw new Error("Unknown round_id in this repository.");
